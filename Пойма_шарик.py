...
while not finished:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            table.append(k)
            with open("scoretable.txt") as fail:
                print(type(fail))
                nomer = 0
                for line in fail:
                    number = 0
                    flag = True
                    while flag and number < len(table):
                        if int(line) <= table[number]:
                            number = number +1
                        else:
                            flag = False
                    table.insert(number, int(line))
            fail = open("scoretable.txt", 'w')
            print('records')
